  ...
  return (
    <main className="relative overflow-hidden">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute inset-0 bg-home-gradient" />
      ...
      <section className="relative mx-auto flex min-h-[calc(100vh-120px)] w-full max-w-7xl items-center px-4 pb-20 pt-16 sm:px-6 lg:px-8">
        <div className="grid w-full items-center gap-14 lg:grid-cols-[1.15fr_0.85fr]">
          <Reveal className="space-y-8">
            <div className="inline-flex items-center gap-3 rounded-full border border-line/10 bg-surface px-4 py-2 text-sm text-text-soft backdrop-blur">
              <span className="inline-flex h-2.5 w-2.5 rounded-full bg-accent shadow-[0_0_18px_rgb(var(--accent)/0.8)]" />
              {siteConfig.availability}
            </div>

            <div className="space-y-6">
              <p className="text-sm uppercase tracking-[0.38em] text-text-muted">
                {profile.role}
              </p>
              <h1 className="max-w-4xl font-display text-5xl font-semibold tracking-tight text-text sm:text-6xl lg:text-7xl">
                Modern interfaces with a futuristic edge and a human touch.
              </h1>
              <p className="max-w-2xl text-lg leading-8 text-text-soft sm:text-xl">
                {profile.heroBlurb}
              </p>
            </div>
          ...
          <Reveal className="relative lg:justify-self-end" delay={100}>
            <div className="relative mx-auto flex aspect-square w-full max-w-[30rem] items-center justify-center">
              <div className="absolute inset-10 rounded-full border border-line/10 bg-surface backdrop-blur-md" />
              <div className="absolute inset-[18%] rounded-full border border-accent/25 bg-[radial-gradient(circle,_rgb(var(--line)/0.16),_transparent_60%)] shadow-glow" />
              <div className="absolute left-1/2 top-1/2 h-5 w-5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-accent shadow-[0_0_40px_rgb(var(--accent)/0.65)]" />
              <div className="absolute left-1/2 top-1/2 h-4 w-4 -translate-x-1/2 -translate-y-1/2 animate-orbit rounded-full bg-secondary" />
              <div className="absolute bottom-12 left-8 animate-float rounded-[28px] border border-line/10 bg-panel/90 px-5 py-4 shadow-soft backdrop-blur">
                <p className="text-xs uppercase tracking-[0.26em] text-accent/80">
                  Interface energy
                </p>
                <p className="mt-2 max-w-[14rem] text-sm leading-6 text-text-soft">
              ...
              <div className="absolute right-4 top-10 rounded-full border border-line/10 bg-surface px-4 py-2 text-sm text-text-soft shadow-soft">
              ...
              <div className="absolute bottom-16 right-10 h-6 w-6 rounded-full bg-secondary/[0.85] shadow-[0_0_30px_rgb(var(--secondary)/0.6)]" />
              <div className="absolute left-10 top-14 h-4 w-4 rounded-full bg-tertiary/[0.85] shadow-[0_0_24px_rgb(var(--tertiary)/0.5)]" />
            </div>
          </Reveal>
        </div>
      </section>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <Reveal delay={100}>
            <Card className="h-full">
              <p className="text-lg leading-8 text-text-soft">{profile.summary}</p>
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                <div className="rounded-[24px] border border-line/10 bg-surface p-5">
                  ...
                  <p className="mt-2 text-lg text-text">{profile.location}</p>
                </div>
                <div className="rounded-[24px] border border-line/10 bg-surface p-5">
                  <p className="text-xs uppercase tracking-[0.28em] text-text-muted">
                    Working style
            ...
            <Card className="h-full">
              ...
              <div className="mt-6 space-y-4">
                {credentials.map((credential) => (
                  <div
                    className="flex items-start gap-3 rounded-[22px] border border-line/10 bg-surface px-4 py-4"
                    key={credential}
                  >
                    <span className="mt-1 h-2.5 w-2.5 rounded-full bg-accent" />
                    <p className="text-text-soft">{credential}</p>
                  </div>
                ))}
              </div>
            </Card>
              ...
              <Card className="overflow-hidden">
                <div className="grid gap-8 lg:grid-cols-[0.32fr_0.68fr]">
                  <div className="space-y-3">
                    ...
                    <p className="text-text-soft">{experience.company}</p>
                    <p className="text-sm text-text-muted">{experience.location}</p>
                  </div>

                  <div className="space-y-5">
                    <p className="leading-7 text-text-soft">{experience.summary}</p>
                    <div className="grid gap-3">
                      {experience.bullets.map((bullet) => (
                        <div
                          className="flex gap-3 rounded-[20px] border border-line/8 bg-surface-soft px-4 py-4"
                          key={bullet}
                        >
                          <span className="mt-2 h-2 w-2 rounded-full bg-secondary" />
                          <p className="text-text-soft">{bullet}</p>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2">
            {education.map((item) => (
              <Card key={`${item.institution}-${item.timeline}`}>
                ...
                <h3 className="mt-3 font-display text-2xl text-text">{item.credential}</h3>
                <p className="mt-2 text-text-soft">{item.institution}</p>
                <p className="mt-4 text-text-muted">{item.notes}</p>
              </Card>
            ))}
          </div>
        )}
      ...
      <section className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8" id="projects">
        ...
        <Reveal className="mt-10" delay={180}>
          <Card className="overflow-hidden p-0">
            ...
            <div className="grid divide-y divide-line/8 md:grid-cols-3 md:divide-x md:divide-y-0">
              {projects.map((project) => (
                <div className="space-y-4 px-6 py-6" key={project.slug}>
                  ...
                  <p className="text-text-soft">{project.summary}</p>
                </div>
              ))}
            </div>
          </Card>
